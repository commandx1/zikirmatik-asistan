...
function options(method: "GET" | "PATCH" | "DELETE", body?: unknown) {
  return {
    method,
    body,
    auth: true as const,
...
export async function getUserById(userId: string): Promise<BackendUser> {
  return request<BackendUser>(`/v1/users/${userId}`, options("GET"));
}
...
export async function saveUserOnboarding(
  ...
  return request<BackendUser>(`/v1/users/${userId}/onboarding`, options("PATCH", payload));
}
...
export async function saveUserPreferences(
  ...
  return request<BackendUser>(`/v1/users/${userId}/preferences`, options("PATCH", payload));
}

export async function deleteUser(userId: string): Promise<void> {
  await request<unknown>(`/v1/users/${userId}`, options("DELETE"));
}
